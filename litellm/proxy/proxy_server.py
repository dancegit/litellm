...
async def async_data_generator(response, user_api_key_dict: UserAPIKeyAuth, request_data: dict):
    ...
    try:
        ...
        # FAKE STREAMING SUPPORT for force_non_streaming
        # When force_non_streaming=True forces stream=False upstream, we get a ModelResponse
        # instead of an async iterator. Wrap it to convert to streaming format.
        from litellm.utils import ModelResponseIterator

        if isinstance(response, ModelResponse) and not hasattr(response, "__aiter__"):
            verbose_proxy_logger.debug("Converting non-streaming ModelResponse to fake streaming")
            response = ModelResponseIterator(response, convert_to_delta=True)
