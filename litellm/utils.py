...
class ModelResponseIterator:
    def __init__(self, model_response: ModelResponse, convert_to_delta: bool = False):
        if convert_to_delta is True:
            ...
            _message = model_response.choices[0].message
            has_tool_calls = False
            if hasattr(_message, "tool_calls") and _message.tool_calls:
                has_tool_calls = True
                # Add index field required for streaming tool_calls format
                tool_calls_with_index = []
                for idx, tc in enumerate(_message.tool_calls):
                    tc_dict = tc.model_dump() if hasattr(tc, "model_dump") else tc.__dict__
                    if "index" not in tc_dict:
                        tc_dict["index"] = idx
                    tool_calls_with_index.append(tc_dict)
                _delta.tool_calls = tool_calls_with_index
            # Set finish_reason: "tool_calls" if tool calls present, "stop" otherwise
            # Also preserve original finish_reason from non-streaming response if available
            original_finish_reason = getattr(model_response.choices[0], "finish_reason", None)
            if original_finish_reason:
                self.model_response.choices[0].finish_reason = original_finish_reason
            elif has_tool_calls:
                self.model_response.choices[0].finish_reason = "tool_calls"
            else:
                self.model_response.choices[0].finish_reason = "stop"
        else:
